...
### For all schemas other than default users
SET PAGESIZE 5000 LINESIZE 220 TRIMSPOOL ON TAB OFF
COLUMN OWNER           FORMAT A24              HEADING 'Schema'
COLUMN SEGMENT_TYPE    FORMAT A20              HEADING 'Segment Type'
COLUMN SIZE_GB         FORMAT 999,990.000      HEADING 'Size (GB)'
COLUMN SIZE_MB         FORMAT 999,999,999      HEADING 'Size (MB)'
COLUMN SEGMENT_COUNT   FORMAT 999,999,999      HEADING 'Segments'
COLUMN HR_SIZE         FORMAT A12              HEADING 'Human Size'
...
WITH system_schemas AS (
  ...
  SELECT 'XS$NULL' FROM DUAL
),
agg AS (
  SELECT
    s.OWNER,
    s.SEGMENT_TYPE,
    SUM(s.BYTES) AS BYTES_TOTAL,
    COUNT(*)     AS SEGMENT_COUNT
  FROM DBA_SEGMENTS s
  WHERE s.OWNER NOT IN (SELECT owner FROM system_schemas)
    -- Optional: exclude UNDO/TEMP segment types
    -- AND s.SEGMENT_TYPE NOT IN ('UNDO', 'TEMPORARY', 'TEMPFILE')
  GROUP BY s.OWNER, s.SEGMENT_TYPE
)
SELECT
  OWNER,
  SEGMENT_TYPE,
  ROUND(BYTES_TOTAL/1024/1024/1024, 3) AS SIZE_GB,
  ROUND(BYTES_TOTAL/1024/1024     , 0) AS SIZE_MB,
  SEGMENT_COUNT,
  CASE
    WHEN BYTES_TOTAL >= POWER(1024,3) THEN TO_CHAR(ROUND(BYTES_TOTAL/POWER(1024,3), 2)) || ' GB'
    WHEN BYTES_TOTAL >= POWER(1024,2) THEN TO_CHAR(ROUND(BYTES_TOTAL/POWER(1024,2), 2)) || ' MB'
    WHEN BYTES_TOTAL >= 1024          THEN TO_CHAR(ROUND(BYTES_TOTAL/1024, 2))           || ' KB'
    ELSE TO_CHAR(BYTES_TOTAL) || ' B'
  END AS HR_SIZE
FROM agg
